...
public class ConfigHelper {
    ...
    public static final String FOLDER_SH3H = "sh3h";
    /**
     * 根目录
     */
//    public static final String FOLDER_ROOT = "sh3h/mainshell";
    public static final String FOLDER_ROOT = "sh3h/mainshell2";
    /**
     * 配置文件目录
     */
...
//            }
    ...
    public File getRootFolderPath() {
        return new File(STORAGE_PATH, FOLDER_ROOT);
    }
     ...
     * @return file path
     */
    public File getLogFilePath(String suffix) {
        ...
        String name;
        if (TextUtil.isNullOrEmpty(suffix)) {
            int pid = android.os.Process.myPid();
            name = String.format(Locale.CHINESE, "log-%s-%d.txt",
                    TextUtil.format(new Date(), TextUtil.FORMAT_DATE), pid);
        } else {
            name = String.format(Locale.CHINESE, "log-%s-%s.txt",
                    TextUtil.format(new Date(), TextUtil.FORMAT_DATE), suffix);
        }
    ...
    public File getApksFolderPath() {
        if (isUsingRoleFunction()) {
            UserSession userSession = mPreferencesHelper.getUserSession();
            return new File(STORAGE_PATH, FOLDER_APKS + "/" + userSession.getUserId());
        } else {
            return new File(STORAGE_PATH, FOLDER_APKS);
        }
    }
...
//    }
    ...
    public String getReservedBrokerUrl() {
        SystemConfig systemConfig = getSystemConfig();
        return systemConfig.getString(SystemConfig.PARAM_RESERVED_BROKER_URL);
    }
    ...
    public int getKeepAliveReservedCount() {
        SystemConfig systemConfig = getSystemConfig();
        return systemConfig.getInteger(SystemConfig.PARAM_KEEP_LIVE_RESERVED_COUNT,
                SystemConfig.KEEP_LIVE_RESERVED_DEFAULT_COUNT);
    }
...
//    }
    ...
    public Observable<Boolean> saveNetWorkSetting(String baseUri, String reservedBaseUri,
        ...
        systemConfig.set(SystemConfig.PARAM_SERVER_BASE_URI,
                baseUri);
        systemConfig.set(SystemConfig.PARAM_RESERVED_SERVER_BASE_URI,
                reservedBaseUri);
        systemConfig.set(SystemConfig.PARAM_BROKER_URL,
                brokeUrl);
        systemConfig.set(SystemConfig.PARAM_RESERVED_BROKER_URL,
                reservedBrokeUrl);
...
//    }
    ...
    public String getGpsType() {
        return getSystemConfig().getString(SystemConfig.PARAM_SYS_GPS_TYPE);
    }

    public Observable<Boolean> saveGpsType(String typeListValue) {
        SystemConfig systemConfig = getSystemConfig();
        systemConfig.set(SystemConfig.PARAM_SYS_GPS_TYPE, typeListValue);
        return Observable.create(new Observable.OnSubscribe<Boolean>() {
            @Override
            public void call(Subscriber<? super Boolean> subscriber) {
                boolean b = getSystemConfig().writeProperties(getSystemConfigFilePath().getPath());
    ...
    public boolean isUsingRoleFunction() {
        return getSystemConfig().getBoolean(SystemConfig.PARAM_USING_ROLE_FUNCTION, false);
    }

    public boolean isUmengPush() {
        SystemConfig systemConfig = getSystemConfig();
        String value = systemConfig.getString(SystemConfig.PARAM_SYS_PUSH_TYPE);
        return !TextUtil.isNullOrEmpty(value) && value.equals(SystemConfig.SYS_PUSH_TYPE_UMENG);
    }
    ...
    public Observable<Boolean> wipeData(final boolean isClearingCache) {
        return Observable.create(new Observable.OnSubscribe<Boolean>() {
            @Override
            public void call(Subscriber<? super Boolean> subscriber) {
                if (subscriber.isUnsubscribed()) {
                ...
                try {
                    String rootPath = STORAGE_PATH + File.separator + FOLDER_SH3H;
                    if (isClearingCache) {
                        findThenDeleteFolders(rootPath, "data", null);
                        findThenDeleteFolders(rootPath, "images", null);
                        findThenDeleteFolders(rootPath, "log", null);
                        findThenDeleteFolders(rootPath, "sounds", null);
                        findThenDeleteFolders(rootPath, "update", getUpdateFolderPath().getPath());
                    } else {
    ...
    public String getLocationUrl() {
        SystemConfig systemConfig = getSystemConfig();
        return systemConfig.getString(SystemConfig.PARAM_LOCATION_URL);
    }
...
//        });
//    }
    private void findThenDeleteFolders(String rootPath, String folderName, String exceptFolder) {
        try {
            String[] names = new File(rootPath).list();
            for (String name : names) {
                File file;
                if (rootPath.endsWith(File.separator)) {
                    file = new File(rootPath + name);
                } else {
                    file = new File(rootPath + File.separator + name);
                }

                if (!file.isDirectory() || (exceptFolder != null && file.getPath().equals(exceptFolder))) {
                    continue;
                }
                ...
                if (name.equalsIgnoreCase(folderName)) {
                    ...
                    findThenDeleteFolders(rootPath + File.separator + name, folderName, exceptFolder);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public boolean needBaiduTrack() {
        //SystemConfig systemConfig = getSystemConfig();
        return false; //systemConfig.getBoolean(SystemConfig.SYS_BAIDU_TRACK, false);
    }

    public int getLocationScanSpan() {
        SystemConfig systemConfig = getSystemConfig();
        return systemConfig.getInteger(SystemConfig.SYS_LOCATION_SCAN_SPAN, SystemConfig.LOCATION_SCAN_SPAN_DEFAULT);
    }
}
