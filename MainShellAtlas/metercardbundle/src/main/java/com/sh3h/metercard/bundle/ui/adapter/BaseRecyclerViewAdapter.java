package com.sh3h.metercard.bundle.ui.adapter;
...
/**
 * create by tanghuijuan,universal Adpter for recyclerView
 *
 * @param <T>
 */
public abstract class BaseRecyclerViewAdapter<T> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private boolean mFootAndEmptyEnable;//底部
    ...
    public static final int FOOTER_VIEW = 0x00000333;//有底部和空视图，但是没有头部视图（position为1）
    public static final int EMPTY_VIEW = 0x00000555;//头部不为空，空视图不为空
    ...
    public class OnItemChildLongClickListener implements View.OnLongClickListener {
        ...
        public boolean onLongClick(View v) {
            if (mChildLongClickListener != null) {
                return mChildLongClickListener.onItemChildLongClick(BaseRecyclerViewAdapter.this, v, mViewHolder.getLayoutPosition() - getHeaderViewsCount());
            }
            return false;
        }
    }
    ...
    public BaseRecyclerViewAdapter(View contentView, List<T> data) {
        this(0, data);
   ...
   /*
    ...
    public int getItemCount() {
        ...
        if (mData.size() == 0 && mEmptyView != null) {
             ...
             */
            if (count == 0 && (!mFootAndEmptyEnable)) {
                count += getmEmptyViewCount();
                /**
                 * {@link #setEmptyView(true, true, View)}
                 */
            } else if (mFootAndEmptyEnable) {
                count += getmEmptyViewCount();
            }

            if ((getHeaderLayoutCount() == 1 && count == 1) || count == 0) {
                mEmptyEnable = true;
                count += getmEmptyViewCount();
            }
        }
    ...
    public int getItemViewType(int position) {
        ...
        if (mData.size() == 0 && mEmptyEnable && mEmptyView != null && position <= 2) {
             ...
             */
            if ((mFootAndEmptyEnable) && position == 1) {
                /**
            ...
            } else if (position == 2 && (mFootAndEmptyEnable) && mHeaderLayout != null && mEmptyView != null) {
                return FOOTER_VIEW;

            } /**
             * user forget to set {@link #setEmptyView(boolean, boolean, View)}  but add footview and headview and emptyview
             */
            else if ((!mFootAndEmptyEnable) && position == 1 && mFooterLayout != null) {
                return FOOTER_VIEW;
            }
        } else if (mData.size() == 0 && mEmptyView != null && mEmptyEnable) {
            return EMPTY_VIEW;
    ...
    private BaseViewHolder getLoadingView(ViewGroup parent) {
        ...
        return createBaseViewHolder(parent,mLoadingViewId);
    }
