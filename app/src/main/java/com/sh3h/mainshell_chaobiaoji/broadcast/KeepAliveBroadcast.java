...
//          getAppInfo(context);
                ...
                sendData();
                checkGpsOpened();
                mMainApplication.checkMessageCache();
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void getLocation() {
        ...
        String appChannel = MainApplication.get(mMainApplication).getAppChannel();
        ...
        if ((!isGpsLocated) || (mrLocation == null)) {
            ...
            if (!TextUtil.isNullOrEmpty(appChannel)
                ...
                mLocation.setLongitude(mrLocation.getLocalLongitude());
                mLocation.setLatitude(mrLocation.getLocalLatitude());
                mLocation.setLocationType("chengjian");
            } else {
    ...
    private void getDeviceToken() {
        ...
        try {
            ...
            jsonObject.put("deviceToken", mPreferencesHelper.getUserSession().getUmengToken());
    ...
    private void checkGpsOpened() {
        ...
        LocationManager locationManager = (LocationManager) mMainApplication.getSystemService(Context.LOCATION_SERVICE);
        ...
        if (!gps && !network) {
            ApplicationsUtil.showMessage(mMainApplication, R.string.text_gps_not_opened);
            mMainApplication.sendGpsNotOpenedMessage();
        }
    }
