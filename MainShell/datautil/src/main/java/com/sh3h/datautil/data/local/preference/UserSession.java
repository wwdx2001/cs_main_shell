...
public class UserSession {
    ...
    private static final String UMENG_TOKEN = "umengToken";
    ...
    private void readSharedPreferences() {
        ...
        if (sp != null) {
            ...
            setAccessToken(sp.getString(ACCESS_TOKEN, TextUtil.EMPTY));
            setUmengToken(sp.getString(UMENG_TOKEN, TextUtil.EMPTY));
            setDepartmentName(sp.getString(KEY_DEPARTMENT_NAME, TextUtil.EMPTY));
            setDepartmentId(sp.getInt(KEY_DEPARTMENT_ID, 0));
        }
    ...
    public boolean save() {
        ...
        if (TextUtil.isNullOrEmpty(accessToken)) {
            editor.putString(ACCESS_TOKEN, TextUtil.EMPTY);
        } else {
            editor.putString(ACCESS_TOKEN, accessToken);
        }

        if (TextUtil.isNullOrEmpty(umengToken)) {
            editor.putString(UMENG_TOKEN, TextUtil.EMPTY);
        } else {
            editor.putString(UMENG_TOKEN, umengToken);
        }

        if (TextUtil.isNullOrEmpty(_departmentName)) {
            editor.putString(KEY_DEPARTMENT_NAME, TextUtil.EMPTY);
        } else {
            editor.putString(KEY_DEPARTMENT_NAME, _departmentName);
        }

        editor.putInt(KEY_DEPARTMENT_ID, _departmentId);
